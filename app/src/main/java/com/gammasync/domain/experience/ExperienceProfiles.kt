...
/**
 * Predefined experience profiles based on research specifications.
 * Each profile is configured for its specific wellness purpose.
 */
object ExperienceProfiles {
     ...
     */
    val NEUROSYNC = ExperienceProfile(
        mode = ExperienceMode.NEUROSYNC,
     ...
     */
    val MEMORY_WRITE = ExperienceProfile(
        mode = ExperienceMode.MEMORY_WRITE,
     ...
     */
    val SLEEP_RAMP = ExperienceProfile(
        mode = ExperienceMode.SLEEP_RAMP,
    ...
    /**
     * Migraine Relief: Static green light experience.
     ...
     */
    val MIGRAINE = ExperienceProfile(
        mode = ExperienceMode.MIGRAINE,
     ...
     */
    val MOOD_LIFT = ExperienceProfile(
        mode = ExperienceMode.MOOD_LIFT,
        audioMode = AudioMode.BINAURAL,
    ...
    /**
     * Get profile for a given experience mode.
     */
    fun forMode(mode: ExperienceMode): ExperienceProfile = when (mode) {
        ExperienceMode.NEUROSYNC -> NEUROSYNC
        ExperienceMode.MEMORY_WRITE -> MEMORY_WRITE
        ExperienceMode.SLEEP_RAMP -> SLEEP_RAMP
        ExperienceMode.MIGRAINE -> MIGRAINE
        ExperienceMode.MOOD_LIFT -> MOOD_LIFT
    }

    /**
     * Get all available profiles.
     */
    fun all(): List<ExperienceProfile> = listOf(
        NEUROSYNC,
        MEMORY_WRITE,
        SLEEP_RAMP,
        MIGRAINE,
     ...
     */
    fun phoneCompatible(): List<ExperienceProfile> = all().filter { !it.requiresXreal }
}