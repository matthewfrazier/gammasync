...
/**
 * Predefined entrainment profiles based on research specifications.
 * Each profile is configured for its specific entrainment purpose.
 */
object EntrainmentProfiles {
     ...
     */
    val NEUROSYNC = EntrainmentProfile(
     ...
     */
    val MEMORY_WRITE = EntrainmentProfile(
        mode = EntrainmentMode.MEMORY_WRITE,
     ...
     */
    val SLEEP_RAMP = EntrainmentProfile(
        mode = EntrainmentMode.SLEEP_RAMP,
    ...
    /**
     * Focus Relief: Static green light entrainment.
     * - No visual flicker (static 525nm green)
     * - Deep delta audio (1Hz) + brown noise for relaxation
     * - Low brightness (<20 nits)
     *
     * Research: Harvard Medical School green light studies for focus enhancement
     */
    val FOCUS_RELIEF = EntrainmentProfile(
        mode = EntrainmentMode.FOCUS_RELIEF,
    ...
    /**
     * Mood Enhancement: Split-field asymmetric stimulation.
     * - Left eye/ear: 18Hz (frontal beta)
     * - Right eye/ear: 10Hz (alpha)
     * - Requires XREAL glasses for independent eye stimulation
     * - Dual binaural audio
     *
     * Research: Frontal alpha asymmetry and mood regulation studies
     */
    val MOOD_ENHANCEMENT = EntrainmentProfile(
        mode = EntrainmentMode.MOOD_ENHANCEMENT,
        audioMode = AudioMode.BINAURAL,
    ...
    /**
     * Get profile for a given entrainment mode.
     */
    fun forMode(mode: EntrainmentMode): EntrainmentProfile = when (mode) {
        EntrainmentMode.NEUROSYNC -> NEUROSYNC
        EntrainmentMode.MEMORY_WRITE -> MEMORY_WRITE
        EntrainmentMode.SLEEP_RAMP -> SLEEP_RAMP
        EntrainmentMode.FOCUS_RELIEF -> FOCUS_RELIEF
        EntrainmentMode.MOOD_ENHANCEMENT -> MOOD_ENHANCEMENT
    }

    /**
     * Get all available profiles.
     */
    fun all(): List<EntrainmentProfile> = listOf(
        NEUROSYNC,
        MEMORY_WRITE,
        SLEEP_RAMP,
        FOCUS_RELIEF,
     ...
     */
    fun phoneCompatible(): List<EntrainmentProfile> = all().filter { !it.requiresXreal }
}